/*
    ...
    * e.g.: ensuring that a write to a buffer completes before reading from it
    * 
    * image layout:
    * 每种layout都可能为一个图像内存的指定用法提供最优的性能
    * 不同的layout之间可以通过使用image barriers进行转换
    * 
    * imgae view:
    * with the swap chain images and the framebuffer, 

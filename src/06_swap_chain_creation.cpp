/*
    ...
    * 
    *   - 图像资源：存储实际像素数据的底层对象。
        - 图像视图：定义了如何查看和访问这些图像资源的一种方式。
        - 附件：每个附件都通过图像视图引用一个图像，并定义如何在渲染过程中使用该图像。
                附件类型通常包括颜色附件、深度附件和模板附件
        - 帧缓冲区：由一个或多个附件组成，定义了渲染操作的目标。帧缓冲区不直接包含图像数据，而是通过附件引用实际的图像资源
        - 交换链：包含多个图像资源，这些资源在渲染过程中轮流用作帧缓冲区的颜色附件。

    *   在渲染流程中，图像视图通过附件方式绑定到帧缓冲区，帧缓冲区定义了渲染操作的目标。
        交换链管理这些图像资源，确保渲染内容可以被有效地显示到屏幕上。
        这种结构化的管理方式使得渲染过程既高效又灵活，允许高质量的图形输出同时保持良好的性能
    * 
    *  - Swap chain creation

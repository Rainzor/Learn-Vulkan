/*
    Previous:   We are required to wait on the previous frame to finish 
                before we can start rendering the next which results in unnecessary idling of the host.

    Solution:   We will create multiple frames in-flight and use fences to synchronize the host periodically.

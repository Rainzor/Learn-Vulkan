...
private:
    ...
    VkSemaphore imageAvailableSemaphore;//信号量句柄
    VkSemaphore renderFinishedSemaphore;//信号量句柄
...
private:
    ...
    void cleanup() {
        ...
        vkDestroySemaphore(device, renderFinishedSemaphore, nullptr);//销毁渲染完成信号量
        vkDestroySemaphore(device, imageAvailableSemaphore, nullptr);//销毁图像可用信号量
        vkDestroyFence(device, inFlightFence, nullptr);//销毁FlightFence
    ...
    void createCommandBuffer() {
        ...
        //level成员变量用于指定分配的指令缓冲的级别:
        //VK_COMMAND_BUFFER_LEVEL_PRIMARY：指令缓冲可以被提交到队列中执行，但不能被其他指令缓冲调用
    ...
    void createSyncObjects() {
        ...
        fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT;//确保创建时已经处于signaled状态
    ...
    void drawFrame() {
        ...
        // 3. 获取需要渲染的交换链图像索引
        uint32_t imageIndex;
        // imageAvailableSemaphore,VK_NULL_HANDLE：指定要等待上一帧presentation阶段完成时，发出信号的同步对象
        // imageIndex：指定要获取可用的VkFrameBuffer索引

/*
    ...
    *
    *  - 之前的许多图形API会为管线提供一些默认的状态。在Vulkan不存在默认状态，
    *    所有状态必须被显式地设置，无论是视口大小，还是使用的颜色混合函数都需要显式地指定。
    * 
    *   渲染管线（Render Pipeline）
    *   渲染管线是一系列顺序的图形处理阶段，每个阶段使用前一个阶段的输出作为输入。
    *   这个管线定义了从获取3D模型（顶点数据）到转换为2D屏幕图像的完整过程。
    * 
    *   渲染管线通常分为两类：固定功能管线（Fixed-function Pipeline）和可编程管线（Programmable Pipeline）。
    * 
    *   固定功能管线：早期图形API中，渲染管线的大部分阶段都是预设好的，开发者只能调整有限的参数。例如，OpenGL的早期版本。
    * 
    *   可编程管线：现代图形API支持在管线的多个阶段使用自定义的着色器（Shader），这允许开发者精确控制图形处理的各个环节。
    *              着色器是小的程序，可以在GPU上运行，用于处理图形和图像数据。
    * 
    *  - 本节主要定义了图形管线的固定功能阶段使用的状态信息，比如输入装配，视口，光栅化，颜色混合
    *   

...
private:
    ...
    VkBuffer geomeBuffers;
    VkDeviceMemory geomeBuffersMemory;
...
private:
    ...
    void cleanup() {
        ...
        vkDestroyBuffer(device, geomeBuffers, nullptr);
        vkFreeMemory(device, geomeBuffersMemory, nullptr);
    ...
    // inorder to use memory more efficiently, we're going to create a buffer that's actually backed by memory on the GPU
    // geometry buffer contains the vertex and index buffers
    void createGeomeBuffers(){
        ...
        memcpy((char*)data+sizeof(vertices[0]) * vertices.size(), indices.data(), (size_t) sizeof(indices[0]) * indices.size());
        vkUnmapMemory(device, stagingBufferMemory);

        createBuffer(bufferSize,
                    VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                    geomeBuffers, geomeBuffersMemory);
        
        copyBuffer(stagingBuffer, geomeBuffers, bufferSize);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        //*-----------------------------------绘制图像-----------------------------------*//
        VkBuffer vertexBuffers[] = {geomeBuffers};
        VkDeviceSize offsets[] = {0};
        //绑定顶点缓冲区
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
        //绑定索引缓冲区
        vkCmdBindIndexBuffer(commandBuffer, geomeBuffers, sizeof(vertices[0]) * vertices.size(), VK_INDEX_TYPE_UINT16);
        //绘制图像
